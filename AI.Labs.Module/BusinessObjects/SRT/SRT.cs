...
namespace AI.Labs.Module
{
    ...
    public class SRTFile
    {
        ...
        public void Save()
        {
            ...
            SRTHelper.CreateSubtitleStream(FileName, t =>
            {
                foreach (var item in Texts.OrderBy(t => t.Index))
                {
                    t.WriteSubtitleItem(item.Text, item.StartTime, item.EndTime, UseIndex ? item.Index : null);
                }
            });
            // 保存到新的SRT文件中
